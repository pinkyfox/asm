...
;Interrupt Service Routine
	cli 						;Запрещаем прерывания, чтобы не произошло прерывания в момент изменения
	                            ;адреса, т. к. возможен крах системы  
	push es 					;сохраняем регистры
	push di
	push ax
;прерывания клавиатуры
;В микропроцессорах Intel 8086/80186 таблица векторов прерываний расположена в первом килобайте памяти
; начиная с адреса 0000:0000 и содержит 256 векторов прерываний в формате сегмент:смещение.  
;Name | Vector |  Address   | Signal source |
;     | number |            |               |
;IRQ1 |  09h   | 0000:0024h |  Клавиатура   |


	xor ax,ax                   ;обнуляем ax 
	mov es,ax                   ;высиавляем es на начало памяти
	mov di,24h					;устанавливаем смещение на прерывание клавиатуры
	mov ax,offset readKeySR 	;заносим в ax адрес функции-обработчика нажатия клавиатуры
	mov es:[di],ax 				;заменяем системною функцию-обработчик нашей функцией
	mov ax,cs 					;сохраняем code segment в ax
	mov es:[di+2],ax 			;заносим сегмент в таблицу векторов прерываний


	;функция-обработчик прерываний таймера
    xor ax,ax 					;ax = 0
    mov es,ax     				;выставляем es на начало таблицы векторов прерываний

    ;переопределяем фукцию-обработчик прерывания 8h
    mov di,20h    				;выставляем смещение на 20h
    mov ax,offset onTimer 		;заносим адрес функции-обработчика прерывания таймера в ax
    mov es:[di],ax 				;заменяем функцию-обработчик прерывания на нашу
    mov ax,cs 					;заносим в ax сегмент, где находится функция обработчик
    mov es:[di+2],ax  			;заносим семент в таблицу вектором-прерываний
...
setUpTimer:                     
    mov al,36h 					;
    out 43h,al 					;устанавливаем режимтаймера
    ;  00    -       11         -      011      -         0
    ;канал 0 -  чтение/запись   - ГПИ(осн. реж) -  формат счетчика
    ;          сначала младшего,                  двоичное 16-битное
    ;          а потом старшего                          число
    ;               байта
    mov al,0ffh
    ...
    mov al,0ffh
    out 40h,al 					;parameter?
    ;таймер настроен на 64000 цикла


    pop ax 						;восстанавливаем регистры
    pop di
    pop es
    sti 	        			;прекращаем блокировку немаскируемых прерываний
;
hang:
	jmp hang
;
...
;**********readKeySR**********
readKeySR:
	cli 					 	;запрещаем немаскируемые прерывания
	...
	push di
  	...
  	in al,60h   				;агрузить данные из порта I/O (60h -- порт клавиатуры) в регистр
  	cmp al,4Bh 					;это ->?
...
RESET_8259:
	mov    al,20h               ;
    out    20h,al    			;отправляем контроллеру прерываний сигнал eoi (end of interrupt)
	...
	sti 						;прекращаем блокировку немаскируемых перываний
...
timedone:
	mov    al,20h               ;уведомляем контроллер прерываний, что
    out    20h,al    			;прерывание закончено
	pop dx  					;восстанавливаем регистры
    pop bx
    pop ax
    pop si
    sti                         ;возобновляем обработку немаскируемы 
    iret                        ;передаем управление из обработчика прерываний
...
wtcont:	
    ...
    pop ax
    pop si
	sti                         ;возобновляем обработку немаскируемых прерываний
	iret                        ;передача управления из функции-обработчика прерывания обратно программе
terminate:
	jmp start
